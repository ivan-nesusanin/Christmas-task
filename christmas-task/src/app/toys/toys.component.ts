...
export class ToysComponent {
  ...
  filterWhite(clickedWhite: boolean): void {
    ...
    if (clickedWhite) {
      this.filterCards = data.filter((item) => item.color === 'белый');
  ...
  filterBlue(clickBlue: boolean): void {
    // if (this.filterCards.length >= 60) this.filterCards = [];
    if (clickBlue) {
      this.filterCards = data.filter((item) => item.color === 'синий');
  ...
  filterRed(clickRed: boolean): void {
    // if (this.filterCards.length >= 60) this.filterCards = [];
    if (clickRed) {
      this.filterCards = data.filter((item) => item.color === 'красный');
  ...
  filterGreen(clickGreen: boolean): void {
    // if (this.filterCards.length >= 60) this.filterCards = [];
    if (clickGreen) {
      this.filterCards = data.filter((item) => item.color === 'зелёный');
  ...
  filterYellow(clickYellow: boolean): void {
    // if (this.filterCards.length >= 60) this.filterCards = [];
    if (clickYellow) {
      this.filterCards = data.filter((item) => item.color === 'желтый');
